...
module divider(
    ...
    // Module-level combinational signals for DONE state (fixes local-var-in-always_ff)
    logic        sign_result_comb;
    logic        sign_remainder_comb;
    logic [XLEN-1:0] quotient_abs_comb;
    logic [XLEN-1:0] remainder_abs_comb;

    always_comb begin
        sign_result_comb    = sign_dividend ^ sign_divisor;
        sign_remainder_comb = sign_dividend;
        quotient_abs_comb   = reg_Q;
        remainder_abs_comb  = reg_A[XLEN-1:0];
    end
    ...
    always_ff @(posedge clk) begin
        if (reset) begin
            ...
            case (state)
                ...
                DONE: begin
                    // Use module-level combinational signals (no local vars in always_ff)
                    final_quotient  <= (sign_result_comb && quotient_abs_comb != 0)
                                       ? -quotient_abs_comb : quotient_abs_comb;
                    final_remainder <= (sign_remainder_comb && remainder_abs_comb != 0)
                                       ? -remainder_abs_comb : remainder_abs_comb;
                    busy  <= 1'b0;
                    done  <= 1'b1;
                    state <= IDLE;
                end
