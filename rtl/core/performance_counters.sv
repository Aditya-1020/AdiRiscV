...
module performance_counters (
    ...
    // Derived metrics: simulation-only (real is not synthesizable)
`ifndef SYNTHESIS
    real ipc;
    real branch_miss_rate;

    always_comb begin
        ipc = (cycles > 0) ? real'(instructions) / real'(cycles) : 0.0;
        branch_miss_rate = (branches > 0) ? real'(branch_misses) / real'(branches) * 100.0 : 0.0;
    end
`endif
